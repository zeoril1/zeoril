...
DISCORD_BOT_TOKEN = 'NjcyMTE5NzA1MjEyOTQ0Mzg1.XjG_1Q.jGSCMJS_J15qWMkZCw57Et6gt1o'
...
async def on_message(message):
    ...
    if message.content.startswith('!member'):
        chan = client.get_channel(message.author.voice.channel.id)
        mem = chan.members
        memname = []  # (list)
        for member in mem:
            memname.append(member.name)
        await message.channel.send(memname)
    ...
    if message.content.startswith('!volume'):
        with open("1.txt", "r") as read_file:
            file_content = read_file.read()
        volume_1 = float(file_content)
        new_volume = message.content
        l = new_volume.find('!')
        r = new_volume.find(' ')
        new_volume = int(new_volume[:l] + new_volume[r+1:])
        global voice
        channel = message.author.voice.channel
        ...
        volume_1 = new_volume/100
        file = open("1.txt", "w")
        file.write(str(volume_1))
        file.close()
        
def play_music(bot,source):
    bot.play(source,after=lambda e: play_next(bot,e))
